...
export interface TimePickerProps {
  /** Selected time (only hour/minute/second are used) */
  value?: Date | null;
  /** Default time */
...
export function TimePicker({
  ...
  const handleInputBlur = () => {
    ...
    const prefix = `${baseDate.getFullYear()}-${String(baseDate.getMonth() + 1).padStart(2, "0")}-${String(baseDate.getDate()).padStart(2, "0")}`;
    const parsed = parseDate(`${prefix} ${inputValue}`, `YYYY-MM-DD ${format}`);
    if (parsed) {
      commitValue(parsed);
    } else {
      setInputValue(value ? formatDate(value, format) : "");
